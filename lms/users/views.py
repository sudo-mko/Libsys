...
def user_reports(request):
    ...
    active_members = users.filter(role='member', membership__isnull=False).count()
    staff_members = users.filter(role__in=['librarian', 'manager', 'admin']).count()
    locked_accounts = users.filter(account_locked_until__isnull=False).count()
    ...
    total_for_percentage = sum(role['count'] for role in role_distribution) or 1
    for role in role_distribution:
        role['percentage'] = round((role['count'] / total_for_percentage) * 100)
        role['name'] = role['role'].title()
        role['role_type'] = 'Staff' if role['role'] in ['librarian', 'manager', 'admin'] else 'Member'

    # Recent activity within the date range
    new_registrations = users.filter(created_at__gte=start_date).count()
    membership_upgrades = users.filter(
        membership__isnull=False,
        created_at__gte=start_date
    ).count()
    account_locks = users.filter(
        account_locked_until__isnull=False,
        last_failed_attempt__gte=start_date
    ).count()
    ...
    if request.GET.get('export') == 'csv':
        ...
        writer.writerow([
            ...
            'Membership', 'Status', 'Created At', 'Last Login'
        ])
        
        for user in users:
            status = 'Locked' if user.account_locked_until else 'Active' if user.is_active else 'Inactive'
            membership = user.membership.name if user.membership else 'None'
            writer.writerow([
                user.username,
                user.first_name,
                user.last_name,
